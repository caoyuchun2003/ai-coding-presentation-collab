...
const slides: Slide[] = [
  ...
  },
  {
    ...
    items: [
      ["上下文", "context-initial.json", "项目初始扫描摘要"],
      ["上下文", "context-question-1.json", "针对单个问题的深挖上下文"],
      ["上下文", "context-htebook-functions.json", "htEbook 功能扫描结果"],
      ["上下文", "context-htebook-cross-projects.json", "htEbook 跨项目关系"],
      ["接口", "function-modules-and-interfaces.md", "模块、Bundle 与接口分布"],
      ["接口", "interface-list.csv", "逐条完整接口清单"],
      ["接口", "interface-module-counts.csv", "接口按模块统计"],
    ],
  },
  {
    kind: "list",
    ...
    items: [
      ["规划", "laravel-refactor-analysis.md", "Laravel 迁移整体分析"],
      ["规划", "laravel-auth-foundation-analysis.md", "未来认证基座分析"],
      ["规划", "laravel-auth-api-contract.md", "未来认证接口契约"],
      ["风险", "response-code-chaos-analysis.md", "当前接口响应混乱风险"],
      ["日志", "operations-log-codex-project-scan.md", "某次项目扫描操作记录"],
  ...
  },
  {
    ...
    cards: [
      ["backups/", "修改前本地备份", "当前示例：junshi_identity_before_20260819.sql。需要回看原始数据或差异时使用。"],
      ["docs/", "专题分析文档", "验证码、Smile 图书、Trae、htAgent。按业务主题组织稳定分析结论。"],
      ["htEbook-sync-package/", "跨项目同步包", "新增/修改分开；再区分公共、AX、ZYX。保留同步范围、来源和交付结构。"],
    ],
  },
  {
    kind: "list",
    ...
    items: [
      ["验证码", "aliyun-captcha-full-migration-plan.md", "阿里云滑动验证码全场景迁移方案"],
      ["验证码", "non-slider-captcha-inventory.md", "传统图片验证码使用清单与迁移顺序"],
      ["图书", "smile-book-resource-structure.md", "Smile 图书资源和目录结构"],
      ["协作", "trae.md", "Trae 工具 / 协作相关记录"],
      ["智能体", "htagent/", "API、认证、数据库、测试完整专题"],
    ],
  },
  {
    kind: "list",
    ...
    items: [
      ["入口", "README.md", "专题导航入口"],
      ["规划", "htagent-redesign-overview.md", "重设计总体说明"],
      ["规划", "htagent-api-redesign.md", "API 重设计方案"],
      ["计划", "htagent-development-tasklist.md", "开发任务拆分"],
      ["计划", "htagent-file-change-plan.md", "预计修改文件清单"],
      ["检查", "htagent-preflight-checklist.md", "开发前检查清单"],
      ["上线", "htagent-phase1-rollout-plan.md", "首期上线与回滚计划"],
      ["复盘", "htagent-final-review-summary.md", "最终审查结论"],
    ],
  },
  {
    kind: "list",
    ...
    items: [
      ["契约", "htagent-api-contract.md", "后端 API 契约"],
      ["前端", "htagent-frontend-api.md", "前端调用方式"],
      ["认证", "htagent-request-auth-flow.md", "请求认证流程"],
      ["认证", "htagent-auth-token-design.md", "Token 设计"],
      ["错误", "htagent-error-response-spec.md", "统一错误响应规范"],
      ["实现", "htagent-symfony33-implementation.md", "Symfony 3.3 落地实现"],
      ["验收", "htagent-frontend-checklist.md", "前端联调与验收清单"],
    ],
  },
  {
    kind: "list",
    ...
    items: [
      ["设计", "htagent-db-schema-design.md", "数据模型与表结构设计"],
      ["DDL", "htagent-ddl-draft.md", "DDL 草案"],
      ["样例", "htagent-api-test-examples.md", "接口请求和响应测试样例"],
      ["Postman", "htagent.postman_collection.json", "可导入的 Postman 请求集合"],
      ["Postman", "htagent.postman_environment.json", "Postman 环境变量模板"],
    ],
  },
  {
    kind: "code",
  ...
  },
  {
    ...
    items: [
      ["01", "读入口", "AGENTS.md / CLAUDE.md · 确认现状和禁区"],
      ["02", "选专题", ".claude/ · 按任务读取清单、风险和历史结论"],
      ["03", "定位代码", "路由 → Controller → Service → Entity / 配置"],
      ["04", "验证留痕", "命令 / Diff / 测试 → 追加 operations-log.md"],
    ],
  },
  {
    kind: "end",
    ...
    title: "先建立事实，\n再让 AI 动手。",
    items: [
      ["AGENTS.md", "入口在哪里、哪些提醒必须先看到？"],
      ["CLAUDE.md", "当前项目、环境、架构、铁律和落库规则？"],
      ["operations-log.md", "某次改动为何发生、本次该怎么追加记录？"],
      [".claude/docs/", "验证码、Smile、htAgent 等专题如何落地？"],
      ["docs/", "正式需求、测试、部署与团队交付是什么？"],
    ],
  },
];

function Topline({ slide }: { slide: Slide }) {
  return (
...
function Heading({ slide }: { slide: Slide }) {
    ...
    <div className="doc-heading">
      <h2>
        {lines.map((line, index) => (
          <span key={`${line}-${index}`}>
            {index > 0 && <br />}
            <em className={index === lines.length - 1 ? "accent" : ""}>{line}</em>
          </span>
        ))}
      </h2>
...
function App() {
  ...
  useEffect(() => {
    const onKey = (event: KeyboardEvent) => {
      ...
      if (event.key === "End") setActive(total - 1);
    };
  ...
  return (
    <main className="doc-app">
      <header className="doc-bar">
        <a className="doc-brand" href="#slide-1">
        ...
        <div className="doc-title">DOCUMENT MAP / DETAILED · 19 SLIDES</div>
      ...
      <div className={`doc-viewport ${slide.kind}`} aria-live="polite">
        <section className="doc-slide" id={`slide-${active + 1}`}>
          {slide.kind === "cover" ? (
            <>
              <div className="doc-eyebrow">
                <span className="live-dot" /> {slide.kicker}
              </div>
            ...
            <>
              ...
              <div className="quick-grid cols-5">
              ...
              <p className="end-summary">
                先建立当前事实，再按任务取上下文，最后用代码和验证收口。文档不是 AI 的附属品，而是 AI 进入真实项目时的第一层工程基础设施。
              </p>
            </>
          ) : (
            <>
              ...
              {slide.kind === "cards" && (
                <div className={`card-grid ${slide.cards?.length === 3 ? "cols-3" : ""}`}>
                  {slide.cards?.map(([tag, name, text]) => (
                    <article key={name}>
                      <span>{tag}</span>
                      <strong>{name}</strong>
                      <p>{text}</p>
              ...
              {slide.kind === "code" && (
                <>
                  <div className={`code-grid ${slide.blocks?.length === 1 ? "single" : ""}`}>
                    {slide.blocks?.map(([tag, name, text]) => (
                      <article key={name}>
                        <span>{tag}</span>
                        <h3>{name}</h3>
                        <pre>{text}</pre>
              ...
              {slide.kind === "facts" && (
                <div className={`facts-grid ${slide.items && slide.items.length > 4 ? "cols-5" : ""}`}>
              ...
              {slide.kind === "track" && (
                <div className="track-grid">
                  {slide.items?.map(([number, name, text]) => (
                    <div key={name}>
                      <b>{number}</b>
                      <strong>{name}</strong>
                      <span>{text}</span>
      ...
      <footer className="doc-controls">
        ...
        <div className="nav-buttons">
          <button onClick={() => go(-1)} disabled={active === 0} aria-label="上一页">
            ←
          </button>
          <button onClick={() => go(1)} disabled={active === total - 1} aria-label="下一页">
            →
          </button>
        </div>
      </footer>
